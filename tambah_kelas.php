		...
		//form untuk menambahkan siswa ke dalam kelas
				...
				echo '<form method="post" action="admin.php?halaman=tambah_kelas" class="form-inline" role="form">';
		...
		//query untuk menampilkan nama2 siswa pada jurusan terkait yang belum mendapatkan/masuk kelas
				...
				echo '</select></div>';
				echo ' <button style="margin-right=20px " type="submit" name="submit" value="simpan" class="btn btn-outline-primary"><span class="glyphicon glyphicon-plus"></span> Tambahkan</button>';
				echo ' <a href="admin.php?halaman=kelas" class="btn btn-link">Daftar Kelas</a>';
				echo '</form>';
				echo '</div></div><br>';
		...
		//tabel daftar siswa
				...
				if(mysqli_num_rows($qkelas) > 0){
					...
					while(list($nis)=mysqli_fetch_array($qkelas)){
						...
						echo '<td>'.$siswa.'</td>';
				//button hapus siswa
						echo '<td><a href="admin.php?halaman=kelas&aksi=view&nis='.$nis.'&kelas='.$kelas.'&tapel='.$tapel.'&idjurusan='.$idjurusan.'&submit=hapus" class="btn btn-danger btn-xs">Hapus</a></td></tr>';
						$no++;
					}
				} else {
...
<form method="post" action="admin.php?halaman=tambah_kelas" class="form-horizontal" role="form">
	...
	<div class="form-group">
		<div class="col-sm-offset-2 col-sm-10">
			<button type="submit" name="submit" value="baru" class="btn btn-outline-primary">Lanjut</button>
